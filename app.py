...
with tab_dashboard:
    ...
    ent_mes_ant = df_mes_passado[df_mes_passado['Tipo'] == 'Entrada']['Valor'].sum() if not df_mes_passado.empty else 0.0
    sai_mes_ant = abs(df_mes_passado[df_mes_passado['Tipo'] == 'Saída']['Valor'].sum()) if not df_mes_passado.empty else 0.0
    rec_mes_ant = df_mes_passado[df_mes_passado['Tipo'].isin(['Entrada', 'Pendência'])]['Valor'].sum() if not df_mes_passado.empty else 0.0

    perc_rec = calc_perc(rec_mes, rec_mes_ant)
    perc_sai = calc_perc(sai_mes, sai_mes_ant)
    perc_saldo = calc_perc(saldo_mes, (ent_mes_ant - sai_mes_ant))

    # BOTÕES DE AÇÃO RÁPIDA
    st.markdown("#### ⚡ Ações Rápidas do Dia")
    col_a1, col_a2, col_a3, col_a4 = st.columns(4)
    with col_a1:
        if st.button("💇‍♀️ Novo Atendimento", type="primary", use_container_width=True, key="btn_modal_atend"):
            dialog_novo_atendimento(servicos)
    with col_a2:
        if st.button("🛍️ Nova Despesa", use_container_width=True, key="btn_modal_despesa"):
            dialog_nova_despesa()
    with col_a3:
        if st.button("💰 Anotar Fiado", use_container_width=True, key="btn_modal_fiado"):
            dialog_anotar_fiado(servicos)
    with col_a4:
        if st.button("💸 Receber Fiado", use_container_width=True, key="btn_modal_baixar"):
    ...
    with k1:
        st.markdown(f'''
            <div class="kpi-card-v2">
                ...
                <div class="kpi-value-v2 kpi-val-pink">R$ {rec_dia:.2f}</div>
                <div class="kpi-perc perc-neutral">Atendimentos de hoje</div>
            </div>
        ''', unsafe_allow_html=True)
    ...
    with k2:
        st.markdown(f'''
            <div class="kpi-card-v2">
                <div class="kpi-title-v2">📈 Faturamento do Mês</div>
                <div class="kpi-value-v2 kpi-val-green">R$ {rec_mes:.2f}</div>
                {render_perc(perc_rec)}
            </div>
        ''', unsafe_allow_html=True)
    ...
    with k3:
        st.markdown(f'''
            <div class="kpi-card-v2">
                <div class="kpi-title-v2">🔻 Despesas do Mês</div>
    ...
    with k4:
        st.markdown(f'''
            <div class="kpi-card-v2">
                <div class="kpi-title-v2">💵 Saldo Líquido do Mês</div>
                <div class="kpi-value-v2 {'kpi-val-green' if saldo_mes >= 0 else 'kpi-val-red'}">R$ {saldo_mes:.2f}</div>
                {render_perc(perc_saldo)}
            </div>
        ''', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # META MENSAL & GRÁFICOS
    c_meta, c_graf1 = st.columns([1, 2])

    with c_meta:
        st.markdown('<div class="ui-card">', unsafe_allow_html=True)
        st.subheader("🎯 Meta de Faturamento Mensal")
        
        meta_valor = st.number_input("Sua Meta para este Mês (R$):", min_value=100.0, value=st.session_state.meta_mensal, step=500.0)
        st.session_state.meta_mensal = meta_valor

        progresso = min(1.0, rec_mes / meta_valor) if meta_valor > 0 else 0.0
        porcentagem = progresso * 100

        st.progress(progresso)
        st.markdown(f"<h3 style='text-align: center; color: #db2777;'>{porcentagem:.1f}% Alcançado</h3>", unsafe_allow_html=True)
        st.markdown(f"<p style='text-align: center; color: #9d174d;'>Faturado: <b>R$ {rec_mes:.2f}</b> / Meta: <b>R$ {meta_valor:.2f}</b></p>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with c_graf1:
        st.markdown('<div class="ui-card">', unsafe_allow_html=True)
        st.subheader("📊 Faturamento por Serviço (Mês Atual)")
        if not df_mes_atual.empty and not df_mes_atual[df_mes_atual['Tipo'] == 'Entrada'].empty:
            df_entradas = df_mes_atual[df_mes_atual['Tipo'] == 'Entrada'].copy()
            df_servs = df_entradas.groupby('Descrição')['Valor'].sum().reset_index()
            fig_servs = px.bar(
                df_servs, x='Valor', y='Descrição', orientation='h',
                color='Valor', color_continuous_scale=['#fbcfe8', '#ec4899', '#831843'],
                text_auto='.2f', labels={'Valor': 'Total (R$)', 'Descrição': 'Serviço'}
            )
            fig_servs.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False, font=dict(color="#831843"))
            st.plotly_chart(fig_servs, use_container_width=True)
        else:
            st.info("Nenhum serviço faturado neste mês.")
        st.markdown('</div>', unsafe_allow_html=True)
...
with tab_servicos:
    st.markdown('<div class="ui-card">', unsafe_allow_html=True)
    st.subheader("⚡ Tabela de Serviços Prestados & Preços")
    st.markdown("<p style='color: #9d174d;'>Cadastre e ajuste o valor dos serviços oferecidos no seu salão.</p>", unsafe_allow_html=True)

    if servicos:
        df_serv_show = pd.DataFrame(list(servicos.items()), columns=["Serviço", "Preço (R$)"])
        df_serv_show["Preço (R$)"] = df_serv_show["Preço (R$)"].map("R$ {:.2f}".format)
        st.dataframe(df_serv_show, use_container_width=True, hide_index=True)
    else:
        st.info("Nenhum serviço cadastrado.")
    st.markdown('</div>', unsafe_allow_html=True)

# ==============================================================================
# TAB 3: CLIENTES VIP / MENSALISTAS / PLANOS
# ==============================================================================
with tab_mensais:
    st.markdown('<div class="ui-card">', unsafe_allow_html=True)
    st.subheader("👥 Cadastro de Clientes VIP e Planos Mensais")
    st.markdown("<p style='color: #9d174d;'>Gerencie pacotes, planos mensais e acertos de clientes fiéis.</p>", unsafe_allow_html=True)

    c_cad_m, c_lista_m = st.columns([1, 2])

    with c_cad_m:
        st.markdown("#### ➕ Cadastrar Cliente VIP")
        with st.form("form_cad_cli_mensal", clear_on_submit=True):
            nome_m = st.text_input("Nome da Cliente:")
            tel_m = st.text_input("WhatsApp (com DDD):")
            if st.form_submit_button("Cadastrar Cliente VIP", type="primary", use_container_width=True):
                if nome_m:
                    cadastrar_cliente_mensal_banco(nome_m, tel_m)
                    st.success("Cliente cadastrada!")
                    st.rerun()
                else:
                    st.warning("Informe ao menos o nome.")

    with c_lista_m:
        st.markdown("#### 📋 Lista de Clientes VIP")
        if not df_clientes_m.empty:
            for _, row in df_clientes_m.iterrows():
                with st.expander(f"✨ {row['Cliente']} — Dívida: R$ {row['Valor Devido']:.2f}"):
                    st.write(f"📱 **WhatsApp:** {row['Telefone']}")
                    st.write(f"💇‍♀️ **Serviços Acumulados:** {row['Serviços Feitos']}")
                    st.write(f"📌 **Status:** {row['Status']}")

                    c_act1, c_act2 = st.columns(2)
                    with c_act1:
                        add_servs = st.number_input("Adicionar Serviços:", min_value=1, value=1, key=f"add_s_{row['id']}")
                        val_un = st.number_input("Valor Un. (R$):", min_value=0.0, value=50.0, step=5.0, key=f"val_s_{row['id']}")
                        if st.button("➕ Lançar Serviços", key=f"btn_add_{row['id']}"):
                            atualizar_cortes_cliente_mensal(row['id'], add_servs, val_un)
                            st.success("Lançamento efetuado!")
                            st.rerun()
                    with c_act2:
                        val_baixa = st.number_input("Valor Baixa (R$):", min_value=0.0, value=float(row['Valor Devido']), key=f"bx_val_{row['id']}")
                        if st.button("💰 Receber Pagamento", key=f"btn_bx_{row['id']}", type="primary"):
                            dar_baixa_divida_mensalista(row['id'], val_baixa)
                            st.success("Pagamento abatido!")
                            st.rerun()
        else:
            st.info("Nenhuma cliente VIP cadastrada.")
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab_agend:
    ...
    st.markdown(f'''
        <div style="background: rgba(244,114,182,0.15); padding: 14px 20px; border-radius: 16px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <b>🔗 Seu Link de Agendamento para Clientes:</b><br>
                <code style="color: #db2777;">{link_clientes}</code>
            </div>
            <a href="{wa_url_geral}" target="_blank" style="background: linear-gradient(135deg, #25d366 0%, #128c7e 100%); color: white; padding: 10px 18px; border-radius: 12px; text-decoration: none; font-weight: bold; box-shadow: 0 4px 12px rgba(37,211,102,0.3);">
    ...
    if not df_agendamentos_all.empty:
        df_ag_disp = df_agendamentos_all.copy()
        st.dataframe(df_ag_disp[["Cliente", "Contato/WhatsApp", "Serviço", "Data", "Horário"]], use_container_width=True, hide_index=True)

        st.markdown("---")
        st.markdown("#### 🗑️ Excluir / Finalizar Agendamento")
        ag_del_id = st.selectbox("Selecione o Agendamento para Remover:", options=df_ag_disp['id'].tolist(), format_func=lambda x: f"ID {x} - {df_ag_disp[df_ag_disp['id']==x]['Cliente'].values[0]} ({df_ag_disp[df_ag_disp['id']==x]['Data'].values[0]} às {df_ag_disp[df_ag_disp['id']==x]['Horário'].values[0]})")
        if st.button("Remover Agendamento", type="secondary"):
            deletar_agendamento(ag_del_id)
...
with tab_historico:
    ...
    if not df_fluxo_caixa.empty:
        df_f_show = df_fluxo_caixa.copy()
        df_f_show['Data'] = df_f_show['Data'].dt.strftime('%d/%m/%Y')
        df_f_show['Valor'] = df_f_show['Valor'].map("R$ {:.2f}".format)

        st.dataframe(df_f_show[['id', 'Data', 'Tipo', 'Descrição', 'Valor']], use_container_width=True, hide_index=True)

        col_pdf, col_del_f = st.columns(2)
        with col_pdf:
            pdf_bytes = gerar_pdf_contabilidade(df_fluxo_caixa, datetime.now(TZ).strftime('%m/%Y'))
            st.download_button("📥 Baixar Relatório em PDF", data=pdf_bytes, file_name=f"relatorio_caixa_{datetime.now(TZ).strftime('%m_%Y')}.pdf", mime="application/pdf", use_container_width=True)

        with col_del_f:
            id_del_f = st.number_input("ID do Lançamento para Excluir:", min_value=1, step=1)
            if st.button("Excluir Lançamento do Caixa"):
                deletar_movimentacao_fluxo(id_del_f)
